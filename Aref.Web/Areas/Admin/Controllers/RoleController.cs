...
public class RoleController(IRoleService roleService, IPermissionService permissionService) : AdminBaseController
{
    ...
    [ValidateAntiForgeryToken]
    [InvokePermission(PermissionsName.SetPermissionToRole)]
    public async Task<IActionResult> SetPermissionToRole(AdminSetPermissionToRoleViewModel viewModel)
    {
        if (!ModelState.IsValid)
        {
        ...
        var result = await roleService.SetPermissionToRoleAsync(viewModel);
    ...
    [HttpGet]
    [InvokePermission(PermissionsName.SetRoleToUser)]
    ...
    [ValidateAntiForgeryToken]
    [InvokePermission(PermissionsName.SetRoleToUser)]
    public async Task<IActionResult> SetRoleToUser(AdminSetRoleToUserViewModel viewModel)
    {
        if (!ModelState.IsValid) 
            return BadRequest(ErrorMessages.ModelStateNotValid);

        var result = await roleService.SetRoleToUserAsync(viewModel);
